  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex gap-4 mb-8 justify-center">
        <button onClick={() => { setActiveTab('image'); setFile(null); setPreviewUrl(null); setAnalysis(''); }} className={`flex items-center gap-2 px-6 py-3 rounded-xl transition-all ${activeTab === 'image' ? 'bg-eva-glow text-eva-base font-bold' : 'bg-white/5 text-white/60 hover:bg-white/10'}`}>
          <ImageIcon size={20} /> Image Analysis
        </button>
        <button onClick={() => { setActiveTab('video'); setFile(null); setPreviewUrl(null); setAnalysis(''); }} className={`flex items-center gap-2 px-6 py-3 rounded-xl transition-all ${activeTab === 'video' ? 'bg-eva-glow text-eva-base font-bold' : 'bg-white/5 text-white/60 hover:bg-white/10'}`}>
          <Video size={20} /> Video Insights
        </button>
        <button onClick={() => { setActiveTab('audio'); setFile(null); setPreviewUrl(null); setAnalysis(''); }} className={`flex items-center gap-2 px-6 py-3 rounded-xl transition-all ${activeTab === 'audio' ? 'bg-eva-glow text-eva-base font-bold' : 'bg-white/5 text-white/60 hover:bg-white/10'}`}>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center min-h-[300px] border-dashed">
          ...
          {previewUrl && (
            <button
              ...
              disabled={loading}
              className="mt-6 w-full bg-eva-glow text-eva-base py-3 rounded-xl font-bold hover:bg-white transition-all flex justify-center items-center gap-2 shadow-lg shadow-eva-glow/20"
            >
