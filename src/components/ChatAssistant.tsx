  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end pointer-events-none">
      <div className={`pointer-events-auto transition-all duration-300 origin-bottom-right transform ${isOpen ? 'scale-100 opacity-100 translate-y-0' : 'scale-90 opacity-0 translate-y-10 pointer-events-none absolute'}`}>
        <div className="w-[350px] h-[500px] bg-eva-panel border border-eva-border rounded-2xl shadow-2xl flex flex-col overflow-hidden">
          {/* Header */}
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] rounded-2xl p-3 text-sm ${m.role === 'user' ? 'bg-eva-glow text-eva-base font-medium' : 'bg-eva-border text-white/90'}`}>
      ...
      <button
        ...
        className={`mt-4 pointer-events-auto p-4 rounded-full shadow-lg transition-all ${isOpen ? 'bg-red-500/20 text-red-300 rotate-45 border border-red-500/30' : 'bg-eva-glow hover:bg-white text-eva-base shadow-eva-glow/20'}`}
      >
